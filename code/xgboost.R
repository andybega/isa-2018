...
# xgboost with extended var set -------------------------------------------


num_data <- cy %>%
  select(gwcode, year, one_of(voi), starts_with("norm_"), starts_with("itt_alleg"), 
         "itt_RstrctAccess", 
         "gtd_killed", "gtd_events", 
         starts_with("internal_confl"), starts_with("ext_conf"), starts_with("any_conflict")) %>%
...
write_rds(mdlX, path = "output/models/mdl_xgboost_extvars.rds")
...
p <- hp_performance %>%
  ...
  theme_minimal() 
p
ggsave(p, file = "output/figures/xgboost-extvars-hp-tuning.png", height = 8, width = 12)
...
p <- oos_preds %>%
  ...
  ggtitle("XGBoost model yhat vs y") 
p
ggsave(p, file = "output/figures/xgboost-extvars-y-vs-yhat.png", height = 5, width = 12)
...
fit_xgboost <- oos_preds %>%
  ...
  mutate(model_name = "XGBoost") %>%
  select(outcome, model_name, MAE, RMSE, CRPS)
write_csv(fit_xgboost, "output/xgboost-extvars-fit.csv")
...
p <- predictor_importance %>%
  ...
  scale_colour_discrete("Outcome")
p
ggsave(p, file = "output/figures/xgboost-extvars-variable-importance-v1.png", height = 6, width = 10)
...
p <- predictor_importance %>%
  ...
  scale_colour_discrete("Outcome", guide = FALSE)
p
ggsave(p, file = "output/figures/xgboost-extvars-variable-importance-v2.png", height = 6, width = 10)
