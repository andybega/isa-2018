...
if (length(PATH_ITT)==0) PATH_ITT <- "input/itt"
if (PATH_ITT=="") PATH_ITT <- "input/itt"
...
itt_api <- function(what = "allegation count", own_path = PATH_ITT) {
  if (what=="allegation count") out <- itt_get_allegation_count(own_path)
  if (what=="by victim") out <- itt_get_by_victim(own_path)
  out
}

itt_get_allegation_count <- function(path) {
  raw <- read_dta(file.path(path, "data/RevisedSAFINAL_11Mar2013.dta"), encoding = "latin1")
  alleg <- raw %>%
