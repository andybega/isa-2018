...
if (length(PATH_LEGAL)==0) PATH_LEGAL <- "input/mrs-legal"
if (PATH_LEGAL=="") PATH_LEGAL <- "input/mrs-legal"

legal_get <- function(what = "raw", own_path = PATH_LEGAL) {
  if (what=="raw") return(legal_get_raw(own_path))
  if (what=="yearly") return(legal_get_raw(own_path))
}
